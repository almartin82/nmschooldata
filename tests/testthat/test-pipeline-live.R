# ==============================================================================
# LIVE Pipeline Tests for nmschooldata
# ==============================================================================
#
...
test_that("New Mexico DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add New Mexico DOE URL and verify HTTP 200")
})
...
test_that("Can download New Mexico enrollment data file", {
  skip_if_offline()
...
test_that("Can parse New Mexico enrollment file", {
  skip_if_offline()
...
test_that("nmschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- nmschooldata::get_available_years()
  ...
  tryCatch({
    raw <- nmschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- nmschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- nmschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- nmschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- nmschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- nmschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- nmschooldata:::get_cache_path(2024, "enrollment")
