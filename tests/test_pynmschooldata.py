...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = nm.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pynmschooldata as nm
        assert hasattr(nm, 'fetch_enr')
        assert hasattr(nm, 'fetch_enr_multi')
        assert hasattr(nm, 'tidy_enr')
        assert hasattr(nm, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pynmschooldata as nm
        assert hasattr(nm, '__version__')
        assert isinstance(nm.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = nm.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        """Total enrollment is in a reasonable range for New Mexico."""
        import pynmschooldata as nm
        years = get_test_years()
        df = nm.fetch_enr(years['max_year'])
        # Total enrollment in New Mexico varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # NM should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = nm.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = nm.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pynmschooldata as nm
        years = get_test_years()
        df_single = nm.fetch_enr(years['max_year'])
        df_multi = nm.fetch_enr_multi([years['max_year']])
