...
class TestFetchEnr:
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_state' in df.columns and 'grade_level' in df.columns and 'subgroup' in df.columns:
            ...
            if len(total_df) > 0:
                total = total_df['n_students'].iloc[0]
                # NM should have ~320k students
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        """Returns a pandas DataFrame."""
        import pynmschooldata as nm
        df = nm.fetch_enr_multi([2022, 2023])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_more_rows_than_single_year(self):
        ...
        import pynmschooldata as nm
        df_single = nm.fetch_enr(2023)
        df_multi = nm.fetch_enr_multi([2022, 2023])
...
class TestDataIntegrity:
    ...
    def test_consistent_between_single_and_multi(self):
        ...
        import pynmschooldata as nm
        df_single = nm.fetch_enr(2023)
        df_multi = nm.fetch_enr_multi([2023])
    ...
    def test_years_within_available_range(self):
        ...
        import pynmschooldata as nm
        years = nm.get_available_years()
        # Fetch a year in the middle of the range
        mid_year = (years['min_year'] + years['max_year']) // 2
        df = nm.fetch_enr(mid_year)
        assert len(df) > 0
